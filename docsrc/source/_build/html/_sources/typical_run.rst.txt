...
This can be quite time consuming if you would run all 11 tools on all of your data.
Luckily, tests on data sets of variable complexity have shown that a small subset of data is enough to assess tool performance.
Once you have decided which tool or tools to use you can run these tools on your entire dataset.

Stage 1 - Tool selection
----------------------------

To run all 11 tools on a small subset of your data, you can specify how many reads and samples to consider.
Tests showed that 100 reads for 3 representative samples each works for most datasets.

Subset the number of reads
.............................

In the ``config.yaml`` you can enter whether you like to subset the number of reads and how many reads per sample you like to include:
...
If you expect the community structure and composition to differ between treatments, it is recommended to include a sample from each of these treatments at this stage.
...
In this example only two samples are used to test the performance of all 11 tools. 
...
- **Runtime** - Runtime plots showing how long it took for each of the 10 tools to run. This may be a decisive factor for your choice of tool(s) if multiple tools show similar performace based on classification or precision, or if you are looking at a deadline. 
...
At the top of the `config.yaml` file you can limit the number of tools. By default all 11 tools are listed, but you can remove the ones you are not interested in:

.. code-block:: bash

   methods:                           ["dcmegablast","minimap"]

In this example only minimap2 and dcmegablast are used.
...
Of course you can still opt to subsample your complete dataset, for instance when the number of reads vary between samples or when more data was generated than needed to characterize your community.
...
Which again contain the classification barplots and runtime plots.
